...
export default function HomePage() {
  return (
    <>
      ...
      <main>
        {/* ── Hero ──────────────────────────────────────────────── */}
        <section className="bg-gradient-to-b from-[#F0FDF4] to-white py-24 px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center mb-10">
              <Link
                href="/register"
                className="bg-[#2D6A4F] text-white px-8 py-3.5 rounded-2xl text-lg font-medium hover:bg-[#245a42] transition-colors shadow-sm"
              >
                Sitter finden →
              </Link>
              <Link
                href="/register"
                className="border-2 border-[#2D6A4F] text-[#2D6A4F] px-8 py-3.5 rounded-2xl text-lg font-medium hover:bg-[#2D6A4F]/5 transition-colors"
              >
                Als Sitter anbieten →
              </Link>
            </div>
            <div className="flex flex-wrap justify-center gap-3">
              {trustBadges.map((badge) => (
                <span
                  key={badge}
                  className="bg-white border border-green-200 text-[#2D6A4F] text-sm px-4 py-1.5 rounded-full shadow-sm"
                >
                  {badge}
                </span>
              ))}
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="bg-[#F0FDF4] rounded-2xl p-8">
                <h3 className="text-xl font-bold text-[#2D6A4F] mb-6">Als Tierhalter</h3>
                <div className="flex flex-col gap-6">
                  {tierhalterSteps.map((s) => (
                    <div key={s.step} className="flex gap-4">
                      <div className="w-9 h-9 rounded-full bg-[#2D6A4F] text-white flex items-center justify-center font-bold text-sm flex-shrink-0">
                        {s.step}
                      </div>
                      <div>
        ...
        <section className="py-20 px-4 bg-gray-50">
          <div className="max-w-6xl mx-auto">
            <h2 className="text-3xl font-bold text-center text-gray-900 mb-3">Alles dabei</h2>
            <p className="text-center text-gray-500 mb-12">Was MeinTiersitter besonders macht</p>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
              {ratgeberTeaser.map((r) => (
                ...
                >
                  ...
                  <h3 className="font-semibold text-gray-900 mb-2 text-sm">{r.title}</h3>
                ...
                {testimonials.map((t) => (
                  <div key={t.name} className="bg-white rounded-2xl p-5 shadow-sm">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-10 h-10 rounded-full bg-[#F0FDF4] text-[#2D6A4F] font-bold flex items-center justify-center text-sm flex-shrink-0">
                        {t.initials}
                      </div>
                      <div>
                        <div className="font-semibold text-gray-900 text-sm">{t.name}</div>
                        <div className="text-xs text-gray-400">{t.ort}</div>
                      </div>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="border border-gray-200 rounded-2xl p-8">
                <h3 className="text-xl font-bold text-gray-900 mb-2">Sitter werden</h3>
                ...
                <Link
                  href="/register"
                  className="block text-center border-2 border-[#2D6A4F] text-[#2D6A4F] px-6 py-3 rounded-2xl font-medium hover:bg-[#2D6A4F]/5 transition-colors"
                >
                  Jetzt gratis registrieren
                </Link>
              </div>
              {/* Tierhalter Card */}
              <div className="border-2 border-[#2D6A4F] rounded-2xl p-8 shadow-lg">
                <h3 className="text-xl font-bold text-gray-900 mb-2">Tierhalter</h3>
                <div className="text-4xl font-bold text-[#2D6A4F] mb-1">
                  €1,90{' '}
                  <span className="text-xl font-normal text-gray-500">/ Monat</span>
                </div>
                ...
                <Link
                  href="/register"
                  className="block text-center bg-[#2D6A4F] text-white px-6 py-3 rounded-2xl font-medium hover:bg-[#245a42] transition-colors"
                >
                  Jetzt starten
                </Link>
              </div>

    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div style={{ background: 'var(--content-bg)', flex: 1 }}>
        <div className="max-w-6xl mx-auto px-4 py-10">
          <h1 className="text-3xl font-extrabold mb-2 text-gray-900">Tiersitti in Rheinland-Pfalz</h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {(landkreise ?? []).map((lk) => (
              lk.is_active ? (
                ...
                >
                  <div className="font-bold text-sm mb-1 text-gray-900">{lk.landkreis_name}</div>
                  {lk.tierheim_name && (
                    <div className="text-xs text-gray-500">{lk.tierheim_name}</div>
                  )}
                  <div className="text-xs mt-2" style={{ color: '#16a34a' }}>● Aktiv</div>
                </Link>
              ) : (
                <div
                  key={lk.landkreis_slug}
                ...
                >
                  <div className="font-semibold text-sm mb-1 text-gray-900">{lk.landkreis_name}</div>
                  <div className="text-xs text-gray-500 mt-2">Bald verfügbar</div>
                </div>
              )
            ))}
          </div>
